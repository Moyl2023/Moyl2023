...
void  OSTimeDly (INT16U ticks)
{
    ...
    if (OSIntNesting > 0) {                      /* See if trying to call from an ISR   中断调用 不处理               */
        return;
    }
    if (ticks > 0) {                             /* 0 means no delay!                                  */
        OS_ENTER_CRITICAL();
        y            =  OSTCBCur->OSTCBY;        /* Delay current task   保存节拍数到当前控制块                              */
        ...
        OS_EXIT_CRITICAL();
        OS_Sched();                              /* Find next task to run!  执行下一个优先级最高的就绪任务 */
    }
}
/*$PAGE*/
/*
