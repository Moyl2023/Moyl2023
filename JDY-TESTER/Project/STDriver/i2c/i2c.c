...
//	uint8 val; 
//	I2C_Start();
//	I2C_Writebyte(0xa0);  //FM24CL16 只有一片  所以A2 A1 A0都为0   最后1位0表示写，1表示读
...
//}
static uint8 Read_EepromByte(uint32 address)
{//BIT2、bit3是芯片选择，bit1是page选择，bit0是读写选择
	uint8 val; 
	uint8 wraddr,rdaddr;
	uint32 Faddr;
	if(address < 65536)
	...
	I2C_Start();
	I2C_Writebyte(wraddr);  //FM24CL16 只有一片  所以A2 A1 A0都为0   最后1位0表示写，1表示读
...
uint8 Write_Page(uint32 address,uint8 *buf,uint16 length)
{
	...
	{//防止地址跨在两个铁电芯片上
		...
		Write_Page_Cov(address,buf,wrsum);//写在第一个芯片上
		Write_Page_Cov(0x20000,buf+wrsum,length -wrsum);//写在第二个芯片上
	}
	return TRUE;
}
...
uint8 Eeprom_Read(uint8 *buf,uint32 address,uint16 length)
{
	...
	{//防止地址跨在两个铁电芯片上
		...
		Eeprom_Read_Cov(buf,address,rdsum);//写在第一个芯片上
		Eeprom_Read_Cov(buf+rdsum,0x20000,length -rdsum);//写在第二个芯片上
	}
	return TRUE;
}
...
/****************************************************************************************
** 函数名称: Eeprom_Write_I2c(uint8 *buf,uint32 address,uint32 bkpaddress,uint16 length)
** 功能描述: I2C 向指定的地址写一组指定长度的数据
** 输　入: unsigned int address:指定地址
**			unsigned char *buf:写入数据的数组指针
**			unsigned char length:写入数据的长度
...
/****************************************************************************************
** 函数名称: uint8 Eeprom_Read_I2c(uint8 *buf,uint32 address,uint32 bkpaddress,uint16 length)
** 功能描述: I2C 向指定的地址读取一组指定长度的数据
** 输　入: unsigned int address:指定地址
			uint32 bkpaddress:备份数据地址
**			unsigned char *buf:写入数据的数组指针
**			unsigned char length:写入数据的长度
** 输　出: TRUE:正确/FALSE:错误
** 全局变量:
** 描述: 同时读取数据区和备份数据区，如果两个区域的数据相同则读取成功，否则失败
******************************************************************************************/
